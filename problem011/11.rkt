...
(define (max-product grid largest xdir ydir xstart xend ystart yend)
    (let loop ((i xstart) (largest largest))
        (cond
            ((<= i xend)
                (let loop2 ((j ystart) (largest largest)) 
                    (cond
                        ((<= j yend) (begin
                            (let ((prod (* 
                                    (vector-ref (vector-ref grid i) j) 
                                    (vector-ref (vector-ref grid (xdir i 1)) (ydir j 1)) 
                                    (vector-ref (vector-ref grid (xdir i 2)) (ydir j 2)) 
                                    (vector-ref (vector-ref grid (xdir i 3)) (ydir j 3)))))
                                (cond
                                    ((> prod largest) (loop2 (add1 j) prod))
                                    (else (loop2 (add1 j) largest))))))
                        (else (loop (add1 i) largest)))))
            (else largest))))
